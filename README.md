...
- Python packages: `boto3`, `python-dotenv`, `psycopg2-binary`, `python-telegram-bot`, and `selenium`
- AWS SES credentials
...
4. Subscriber records are stored in the configured PostgreSQL database and expire after five days.
...
- Because `main.py` uses Telegram by default, configure `TELEGRAM_BOT_TOKEN`, `MAXIMUM_ENTRIES`, the PostgreSQL connection settings, `ADMIN_ID`, and `ALLOWED_DOMAINS` as workflow secrets and add them to the workflow environment before using the scheduled job.
